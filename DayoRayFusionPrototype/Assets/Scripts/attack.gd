...
func handle_block():
	if (block_cooldown > 0 or player_busy()) and player.attacked == false:
		return
...
func handle_attack():
	if player_busy() and player.attacked == false:
		return
